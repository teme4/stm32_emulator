...
// const uint8_t Configuration_Descriptor[67] = {
// 	// Little Endian.
...
void Send_EP0(const char *data, uint32_t len)
{
	...
	uint32_t ii = 0;
	for (ii = 0; ii < len;)
	{ // Copy data from data buffer to endpoint 0 buffer. Note: endpoint 0 buffer in PMA is 16 bit wide, not 32.
		*p_mem_16 = *data_16;
		p_mem_16 += 2;
		data_16 += 1;
		ii += 2;
...
void Send_EP1(uint8_t *data, uint32_t len)
{
	...
	uint32_t ii = 0;
	for (ii = 0; ii < len;)
	{ // Copy data from data buffer to endpoint 1 buffer. Note: endpoint 1 buffer in PMA is 16 bit wide, not 32.
		*p_mem_16 = *data_16;
		p_mem_16 += 2;
		data_16 += 1;
		ii += 2;
...
{
	if (len < 32)
	{ // If the packet is not bigger than 32 byte use the standard function Send_EP1.
	...
	N_JUMBO_SUBPACKETS_TXED = 1;
} // Set the number of transmitted packets to 1.
...
void Read_EP1(uint8_t *Received_data, uint32_t *len)
{
	...
	{ // Copy the received data from endpoint 1 reception buffer to "Received_data" buffer. Note: endpoint 1 buffer is 16 bit wide, not 32.
	...
	*len = N_Byte_Received; // Copy the lenght of the received packet in the variable *len.
	EN_RX_EP1();
} // Enable the endpoint 1 reception buffer to receive any new packet.
...
{
	if (USB_RECEIVED_EP0)
	{						  // If a packet has been received on endpoint 0..
		...
		if (p_mem_8[0] == 0x80 && p_mem_8[1] == 0x06)
		{ // If a descriptor has been requested..

			if (p_mem_8[5] == 0x01)
			{									 // If REQUEST Device_Descriptor..
				Send_EP0(Device_Descriptor, 18); // Send the Device_Descriptor.
				while (USB_TRANSMITTED_EP0 == 0)
				{
				} // Wait that the packet has been completely transmitted.
				USB_TRANSMITTED_EP0 = 0;
			} // Reset flag trasmission complete.

			else if (p_mem_8[5] == 0x02)
			{ // If REQUEST Configuration_Descriptor..
				if (p_mem_8[12] == 0x09 && p_mem_8[13] == 0x00)
				{										   // If requested the partial Configuration_Descriptor..
				...
				{											  // If requested the complete Configuration_Descriptor..
			...
			else if (p_mem_8[5] == 0x06)
			{ // If REQUEST DEVICE_QUALIFIER_DESCIPTOR..
				STALL_TX_EP0();
			} // Send REQUEST_ERROR (Because USB1.1 doesn't support that).
		}

		else if (p_mem_8[0] == 0x00 && p_mem_8[1] == 0x05)
		...
		else if (p_mem_8[0] == 0x00 && p_mem_8[1] == 0x09)
		...
		else if (p_mem_8[0] == 0xA1 && p_mem_8[1] == 0x21)
		...
		else if (p_mem_8[0] == 0x21 && p_mem_8[1] == 0x22)
		{										// If REQUEST SET_CONTROL_LINE_STATE..
		...
		else if (p_mem_8[0] == 0x21 && p_mem_8[1] == 0x20)
		{				 // If REQUEST SET_LINE_CODING..
			EN_RX_EP0(); // Enable reception on endpoint 0.
			while (USB_RECEIVED_EP0 == 0)
			{
			}; // Wait the reception of the DATA packet.
		...
		else if (p_mem_8[0] == 0x02 && p_mem_8[1] == 0x01)
	...
	if (USB_RECEIVED_EP2)
	{						  // If a packet has been received on endpoint 1..
		USB_RECEIVED_EP2 = 0; // Reset flag reception endpoint 2.
		EN_RX_EP2();
	} // Enable reception on endpoint 0.
}
...
void Manage_EP1_Jumbo()
{
	...
	{
		Send_EP1((uint8_t *)JUMBO_BUFF_ADDR, 32);
	} // If not the last packet send a subpacket of 32 bytes.
	else
	{
		Send_EP1((uint8_t *)JUMBO_BUFF_ADDR, JUMBO_LAST_SUBPACKET_SIZE);
	} // If the last packet send a packet with the remaining bytes.
	N_JUMBO_SUBPACKETS_TXED++;
} // Report that a new packet will be sent.
...
void USB_LP_CAN1_RX0_IRQHandler(void)
{
	...
	if (USB_ISTR & 0x8000)
	{ // If a transmission has been completed correctly..
		if (USB_ENP0R & 0x8000)
		{						  // If the transmission is EP0 OUT..
		...
		if (USB_ENP1R & 0x8000)
		{						  // If the transmission is EP1 OUT..
		...
		if (USB_ENP1R & 0x0080)
		{							 // If the transmission is è EP1 IN..
			...
			if (N_JUMBO_SUBPACKETS > 1)
			{						  // If any jumbo subpacket still need to be transmitted..
				N_JUMBO_SUBPACKETS--; // Report that a new packet has been transmitted.
				Manage_EP1_Jumbo();
			} // Manage the transmission of the new jumbo subpacket.
			else if (N_JUMBO_SUBPACKETS == 1)
			{ // If the last jumbo subpacket has been completely transmitted..
				N_JUMBO_SUBPACKETS = 0;
			}
		} // Report that there isn't any jumbo subpacket to be transmitted.

		if (USB_ENP2R & 0x8000)
		{						  // If the transmission is EP2 OUT..
